...
int main() {
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// 变换矩阵
		glm::mat4 transform = glm::mat4(1.0f);//, rotateMatrix,translateMatrix, scaleMatrix;
		transform = glm::rotate(transform, glm::radians(-55.0f), glm::vec3(1.0f, 0.5f, 0.0f));
		 
		//transform = rotateMatrix;// * translateMatrix * scaleMatrix;
		//transform = glm::translate(transform, glm::vec3(0.0f,0.0f,-3.0f));
		glUniformMatrix4fv(glGetUniformLocation(mShader.ID, "transform"), 1, GL_FALSE, glm::value_ptr(transform));
