...
export default class VerticalScrollView extends cc.Component {
    ...
    // 初始偏移量（用于控制优先显示第几个节点）
    private mStart: number = undefined;

    // 最上面的节点
    private mBegin: number = undefined ;

    // 最下面的节点
    private mEnd: number = undefined; 

    // 最上面的节点模拟的节点计数，从1开始计数
    private mTopIndex: number = undefined;

    // 最下面的节点模拟的节点计数
    private mBottonIndex: number = undefined;

    // 模拟节点最多有几个
    private mMaxIndex: number = undefined;

    // 实际的节点数量
    private mChildCount: number = undefined;

    // 子节点的高度
    private mItemHeight: number = undefined;

    // 触发移动的最小偏移量(子节点高度 + 节点Y轴距离)
    private mBaseOffset: number = undefined;
    
    // 节点的移动量
    private mMove: number = undefined;

    // content子节点数组
    private mNodes: cc.Node[] = []; 
   ...
   public onLoad () {
        ...
        // tContent的
        tContent.resizeMode = cc.Layout.ResizeMode.NONE; 
